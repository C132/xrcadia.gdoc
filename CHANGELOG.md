...
### Changed
- **Sidebar toggle icon.** Replaced the triangle/hamburger icons (◀ / ≡) with the sidebar icon (◧) for a clearer visual metaphor.
- **Refresh button icon.** Replaced the text "Reload" button with a styled refresh icon (⟳) matching the toolbar's icon-based design. Includes hover color feedback consistent with other toolbar buttons.
